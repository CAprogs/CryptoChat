...
from .Database import DatabaseHandler, DB_NAME
...
class Server(User):
    ...
    def handle_client(self, client):
        ...
        DB.insert_user(sender, self.users[client].get("public_key"),
                       self.users[client].get("host"),
                       self.users[client].get("public_ip"),
                       self.users[client].get("city"),
                       self.users[client].get("region"),
                       self.users[client].get("location"),
                       self.users[client].get("timestamp"))
        while True:
            try:
                ...
                if len(self.users) == 1 and verified:
                    ...
                    timestamp = get_timestamp()
                    DB.insert_conversation(sender, receivers_username[0], decrypted_message, timestamp)
                elif len(self.users) > 2 and verified: # Send message to all clients [except sender] if server has more than 2 clients
                    self.broadcast(f"{sender} ▻ {decrypted_message.decode("utf-8")}", receivers_list)
                    timestamp = get_timestamp()
                    DB.insert_conversation(sender, str(receivers_username), decrypted_message, timestamp)
                else:
                    pass
